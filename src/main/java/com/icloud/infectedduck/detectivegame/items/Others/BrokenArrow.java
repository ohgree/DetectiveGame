 ...
 */
public class BrokenArrow extends ItemBase {
    public BrokenArrow() {
        initKillItem(KillType.BLADE);
        initItem("부러진 화살", new ItemStack(Material.ARROW, 1), 0, Arrays.asList(ProfessionBase.Profession.NONE),
                Arrays.asList(ItemTarget.NPC), new String[]{
                        ChatColor.GRAY + "\"활 丕刀卜\"라고 쓰여있다.",
                        ChatColor.GRAY + "한자로 되어있어 읽을수가 없다."
                });
    }
}
