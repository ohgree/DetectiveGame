 ...
 */
public class FlowerPot extends ItemBase {
    public FlowerPot() {
        initKillItem(KillType.MACE);
        initItem("화분", new ItemStack(Material.FLOWER_POT_ITEM, 1), 1, Arrays.asList(ProfessionBase.Profession.NONE),
                Arrays.asList(ItemTarget.NPC), new String[] {
                        ChatColor.GRAY + "이걸로 중력낙하실험을 해선 안됩니다"
                });
        initCorpseMessage("골절로 인한 내장파열, 창상, 자상의 복합적인 상처를 발견했다", "현장 주변에서 깨진 화분조각을 발견했다");
    }
}
