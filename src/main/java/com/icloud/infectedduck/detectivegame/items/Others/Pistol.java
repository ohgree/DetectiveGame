...
public class Pistol extends ItemBase {
    public Pistol() {
        ...
        initItem("권총", new ItemStack(Material.IRON_BARDING, 1), 5, Arrays.asList(ProfessionBase.Profession.NONE),
        ...
        initCorpseMessage("몸에서 깊은 총상을 발견할 수 있었다", "현장 주변에서 뭉개진 권총 탄두 3개를 발견했다");
    }
            ...
            if (e.getAction().equals(Action.RIGHT_CLICK_AIR) || e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
                if (isUserValid(e.getPlayer())) {
                    ...
                    if (DetectiveGameUtils.getTarget(user) instanceof Player) {
                        ...
                        if (!pb.isSilenced()) {
                            target.getWorld().playSound(user.getLocation(), Sound.FIREWORK_BLAST, 10000.0f, 0.3f);
                            target.getWorld().playSound(user.getLocation(), Sound.FIREWORK_LARGE_BLAST, 10000.0f, 0.7f);
